...
**Un commit es un acto legislativo, no un documento afectado** (ver
`agrupar_eventos`): si una ley reformó ocho códigos el mismo día, eso es un
commit que toca los ocho archivos, no ocho commits con el mismo asunto.

Reanudable: el progreso se guarda en `state.json` (ignorado por git), con las
claves de `CommitGroup.key`. Los grupos ya commiteados se saltan, así que el
script se puede detener y relanzar sin problema durante un recorrido largo y
limitado por el rate limit de BCN.
...
@dataclass
class CommitEvent:
    """Un documento en una de sus versiones históricas.

    `link` indica de qué norma de BCN sale este texto, que en un documento con
    linaje no es siempre la misma.

    Un evento **no** es un commit: los eventos que comparten causa y fecha se
    agrupan en un solo `CommitGroup` (ver `agrupar_eventos`).

    `inicio_de_eslabon` se marca cuando este evento estrena un eslabón del
    linaje del documento, para advertirlo en el mensaje del commit.
    """
...
@dataclass
class CommitGroup:
    """Un commit por hacer: un acto legislativo.

    Reúne todos los documentos que una misma norma —o un mismo conjunto de
    normas— modificó en la misma fecha. La Ley 20.720 reformó ocho códigos el
    2014-10-10: eso es este grupo, y produce un solo commit que toca los ocho
    archivos.

    Los eventos sin causa registrada y las publicaciones originales no tienen
    con qué agruparse, así que cada uno forma un grupo de un solo documento.
    """

    events: list[CommitEvent]

    @property
    def representante(self) -> CommitEvent:
        """Cualquier evento del grupo sirve para lo que comparten todos: la
        fecha de vigencia y las normas modificatorias."""
        return self.events[0]

    @property
    def version(self) -> VersionEvent:
        return self.representante.version

    @property
    def fecha(self) -> date:
        return self.version.vigente_desde

    @property
    def key(self) -> str:
        """Identificador estable del grupo, usado en `state.json` para saber
        qué ya se commiteó y poder reanudar.

        Dos formas, según cómo se agrupó (ver `agrupar_eventos`):

            2014-10-10:mods:1058072      un acto legislativo
            1855-01-01:doc:172986        un documento sin causa que agrupar

        La fecha va primero para que `state.json` quede legible en orden
        cronológico.
        """
        fecha = self.fecha.isoformat()
        if self.version.modificatorias:
            ids = "-".join(str(i) for i in sorted(m.id_norma for m in self.version.modificatorias))
            return f"{fecha}:mods:{ids}"
        return f"{fecha}:doc:{self.representante.link.id_norma}"

    @property
    def rutas(self) -> list[Path]:
        return [e.target.path.relative_to(DATA_REPO_ROOT) for e in self.events]


def agrupar_eventos(events: list[CommitEvent]) -> list[CommitGroup]:
    """Junta los eventos que son el mismo acto legislativo.

    La clave de agrupación es **(fecha de vigencia, conjunto exacto de normas
    modificatorias)**. El conjunto tiene que ser exacto, y no "comparten al
    menos una norma", para que se cumpla la invariante que hace legible el
    commit: *cada norma citada modificó cada documento del commit*.

    Un ejemplo real de por qué importa: el 2023-04-10 el Código Penal cambió por
    las leyes 21.555, 21.557 y 21.560, el Código Procesal Penal por la 21.555 y
    la 21.560, y el Código de Justicia Militar sólo por la 21.560. Agrupando por
    "comparten alguna" quedarían los tres documentos y las tres leyes en un solo
    commit, sugiriendo que la Ley 21.557 tocó el Código de Justicia Militar, que
    es falso. Con el conjunto exacto son tres commits, cada uno honesto sobre su
    causa. Medido sobre el corpus, la diferencia entre ambos criterios son
    apenas dos casos (éste y el 2005-02-16), así que la exactitud sale casi
    gratis.

    Lo que **no** se agrupa:

    - Las publicaciones originales y las versiones sin causa registrada por BCN
      (`is_unknown_cause`): no hay norma por la cual juntarlas, y compartir
      fecha no las hace el mismo acto.
    - Una misma norma que empieza a regir sobre distintos documentos en fechas
      distintas. Son 45 de las 728 normas del corpus, y no es un caso sucio sino
      real: la Ley 21.713 rige sobre el Código Tributario en seis fechas
      escalonadas. Cada fecha es un momento distinto de la historia y merece su
      propio commit, en su lugar de la cronología.
    """
    grupos: dict[tuple, list[CommitEvent]] = {}
    for event in events:
        version = event.version
        if version.modificatorias:
            clave = (version.vigente_desde, frozenset(m.id_norma for m in version.modificatorias))
        else:
            # Sin causa que compartir, cada documento va por su cuenta: la clave
            # incluye el eslabón, que es único por documento y fecha.
            clave = (version.vigente_desde, event.link.id_norma)
        grupos.setdefault(clave, []).append(event)

    # Orden estable dentro de cada grupo y entre grupos: primero la fecha (el
    # orden cronológico global del que depende todo lo demás) y después los
    # documentos, para que el resultado no dependa del orden en que BCN los
    # devolvió.
    salida = [CommitGroup(events=sorted(evs, key=lambda e: e.target.slug)) for evs in grupos.values()]
    salida.sort(key=lambda g: (g.fecha, [e.target.slug for e in g.events], g.key))
    return salida
...
def load_state() -> set[str]:
    """Claves de los commits ya hechos (vacío en la primera ejecución)."""
    if not STATE_FILE.exists():
        return set()
    done = set(json.loads(STATE_FILE.read_text()))

    # Guarda contra un `state.json` de antes de agrupar por acto legislativo,
    # cuando la clave era "<id_norma>:<vigente_desde>" y había un commit por
    # documento afectado. Ninguna de esas claves coincide con las de ahora, así
    # que el build volvería a commitear toda la historia encima de la que ya
    # está en el repo de datos: hay que reconstruirlo desde cero.
    antiguas = [k for k in done if ":doc:" not in k and ":mods:" not in k]
    if antiguas:
        raise SystemExit(
            f"{STATE_FILE} viene del formato anterior (un commit por documento afectado; "
            f"{len(antiguas)} claves como '{sorted(antiguas)[0]}').\n"
            "Ahora un commit es un acto legislativo completo, así que la historia hay que "
            "reconstruirla:\n"
            f"  rm {STATE_FILE}\n"
            f"  rm -rf {DATA_REPO_ROOT}/.git {DATA_REPO_ROOT}/codigos {DATA_REPO_ROOT}/constitucion "
            f"{DATA_REPO_ROOT}/normas\n"
            f"  git -C {DATA_REPO_ROOT} init\n"
            "(la caché de BCN se conserva, así que reconstruir no cuesta ni una petición nueva)"
        )
    return done
...
def build_commit_message(
    ...
    *,
    ...
    titulos_documentos: dict[int, str],
    date_clamped: bool,
    ...
    El cuerpo siempre cita la fuente exacta de cada documento para que el commit
    sea auditable, y termina con las líneas `Co-authored-by:` que GitHub usa
    para mostrar a todos los autores y firmantes.
    ...
    `fuentes` y `titulos_documentos` mapean idNorma del eslabón -> URL de BCN de
    la que salió ese texto / título del documento. Van por eslabón y no por
    documento porque un documento con linaje cambia de norma de origen a lo
    ...
    # El título del documento sólo se usa cuando el commit tiene uno solo (una
    # publicación original, un cambio sin causa registrada, un cambio de
    # eslabón): en esos casos no hay norma modificatoria que nombrar en el
    # asunto.
    titulo_norma = titulos_documentos.get(grupo.representante.link.id_norma, "")
    # Un evento por commit puede estrenar un eslabón del linaje; entonces el
    # diff es el reemplazo completo del cuerpo legal anterior, no una reforma.
    estreno = next((e for e in grupo.events if e.inicio_de_eslabon), None)
    inicio_de_eslabon = estreno.inicio_de_eslabon if estreno else None
    ...
    # Los documentos afectados van listados uno por línea, cada uno con la URL
    # exacta de BCN de la que salió su texto en esta versión. Cuando son varios
    # —el caso que motivó agrupar— el conteo adelanta el alcance de la reforma
    # sin tener que leer el diff.
    lines = [
        header,
        "",
    ...
    if len(grupo.events) == 1:
        lines.append(f"Documento: {grupo.rutas[0]}")
        lines.append(f"  Fuente: {fuentes[grupo.representante.link.id_norma]}")
    else:
        lines.append(f"Documentos afectados ({len(grupo.events)}):")
        for evento, ruta in zip(grupo.events, grupo.rutas):
            lines.append(f"  {ruta}")
            lines.append(f"    Fuente: {fuentes[evento.link.id_norma]}")

    # Concordancia con la cantidad de documentos: "modifica el documento" suena
    # a uno solo, y estos commits pueden tocar ocho.
    cuantos = "el documento" if len(grupo.events) == 1 else f"los {len(grupo.events)} documentos"
    for m in version.modificatorias:
        ...
        lines.append(
            f"{_describe(m)} {verbo_para(m.tipo_norma)} {cuantos} "
    ...
    if inicio_de_eslabon is not None:
        lines.append(
            f"\nNota: este commit estrena un nuevo cuerpo legal dentro del linaje de "
            f"{estreno.target.path.relative_to(DATA_REPO_ROOT)}, así que el diff refleja el "
            "reemplazo completo del texto anterior y no una reforma puntual."
        )
    if date_clamped:
        lines.append(
...
def assign_commit_datetimes(grupos: list["CommitGroup"]) -> dict[str, str]:
    """Fecha git de cada commit: la fecha real de vigencia (mediodía, hora de
    Chile).

    Excepción: los grupos anteriores a `GIT_EPOCH` reciben una fecha sintética
    que parte en 1970-01-01T00:00:00Z y avanza un segundo por cada grupo de
    ese tipo, siguiendo el orden de `grupos` (que se asume ya ordenado
    ...
    for grupo in grupos:
        ...
        if d < GIT_EPOCH:
            ...
            result[grupo.key] = synthetic.strftime("%Y-%m-%dT%H:%M:%S+00:00")
            pre_epoch_counter += 1
        else:
            result[grupo.key] = f"{d.isoformat()} 12:00:00 -0300"
    return result
...
def commit_group(
    ...
    """Descarga, renderiza y commitea un acto legislativo completo.

    Un commit puede tocar varios documentos: todos los que la(s) misma(s)
    norma(s) modificaron en esa fecha (ver `agrupar_eventos`).

    El texto de cada documento se pide a `event.link.id_norma`, que en un
    documento con linaje no coincide con "la norma del target": va cambiando de
    eslabón a medida que avanza la historia.

    En el mismo commit se guardan también las normas que causaron el cambio, de
    modo que un commit contenga a la vez el efecto (los códigos modificados) y
    la causa (la ley que los modificó).
    """
    version = grupo.version
    archivos: list[Path] = []
    fuentes: dict[int, str] = {}
    titulos_documentos: dict[int, str] = {}
    doc_representante: NormaDocument | None = None

    for event in grupo.events:
        link = event.link
        url = GET_NORMA_JSON_URL_TEMPLATE.format(
            id_norma=link.id_norma, version_date=version.vigente_desde.isoformat()
        )
        result = client.get(url)
        doc = parse_norma_json(
            result.content, id_norma=link.id_norma, version_date=version.vigente_desde.isoformat()
        )
        event.target.path.parent.mkdir(parents=True, exist_ok=True)
        event.target.path.write_text(render_markdown(doc, source_url=url, fetched_at=result.fetched_at))
        archivos.append(event.target.path)
        fuentes[link.id_norma] = url
        titulos_documentos[link.id_norma] = doc.titulo_norma
        if event is grupo.representante:
            doc_representante = doc

    # Guardar el texto de cada norma modificatoria junto a los documentos que
    # afectó. Se descarga una sola vez por commit, aunque haya tocado ocho
    # códigos: la causa es la misma para todo el grupo.
    ...
    # Los commits sin norma modificatoria —la publicación original y los
    # cambios que BCN no atribuye— tienen siempre un solo documento, así que el
    # representante del grupo *es* el documento.
    doc = doc_representante
    ...
    if version.is_original:
        ...
        resolved = resolve_authors(
            client,
            id_norma=grupo.representante.link.id_norma,
            organismo=organismo,
            promulgacion_doc=doc,
        )
    elif version.is_unknown_cause:
...
def main() -> None:
    ...
    for target in targets:
        ...
        for evento in eventos_del_target:
            ...
            if len(eslabones_vistos) > 1:  # el primero es el inicio natural, no un salto
                evento.inicio_de_eslabon = evento.link

    # Un commit es un acto legislativo, no un documento afectado: los eventos
    # que comparten fecha y causa se juntan en un solo commit.
    grupos = agrupar_eventos(all_events)
    commit_datetimes = assign_commit_datetimes(grupos)
    print(
        f"{len(all_events)} versiones agrupadas en {len(grupos)} commits.",
        file=sys.stderr,
        flush=True,
    )
    ...
    failed: list[str] = []
    for grupo in grupos:
        if grupo.key in done:
            continue
        documentos = ", ".join(e.target.slug for e in grupo.events)
        print(f"Commiteando {grupo.key} ({documentos})...", file=sys.stderr)
        try:
            commit_group(client, grupo, commit_datetimes[grupo.key], catalogo)
        except Exception as exc:  # noqa: BLE001 - un evento malo no puede matar un recorrido de días
            print(f"  FALLÓ {grupo.key}: {exc!r} (se reintentará en la próxima corrida)", file=sys.stderr)
            failed.append(grupo.key)
            continue
        done.add(grupo.key)
        save_state(done)
    ...
    print(
        f"Listo. {len(all_events)} versiones en {len(grupos)} commits, {len(done)} commiteados "
        f"acumulados ({committed_this_run} en esta corrida, {len(failed)} fallidos que se "
        "reintentarán).",
        file=sys.stderr,
    )
